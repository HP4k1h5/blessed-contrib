...
Line.prototype.calcSize = function () {
  // leaving original calculations here, but i find the ensuing more flexible
  // for more chart types
  // this.canvasSize = { width: this.width * 2 - 12, height: this.height * 4 - 8 }
  this.canvasSize = { width: this.width * 2 - 4, height: this.height * 4 }
}
...
Line.prototype.setData = function (data) {
  ...
  var yLabelPadding = 1
  var xPadding = 0 //this.options.xPadding
  var yPadding = 3
  var c = this.ctx
  var labels = data[0].x
  ...
  function getYPixel(val, minY) {
    ...
    res -= 6 //to separate the baseline and the data line to separate chars so canvas will show separate colors
    return res
  }
